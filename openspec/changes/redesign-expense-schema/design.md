...
Amounts are integer minor units with an ISO-4217 currency, never `REAL`, to avoid rounding drift in totals. Each row also stores a base-currency equivalent, the rate used and the rate date, all captured at write time so historical reports do not move when rates change. The base currency is one fixed value for the whole database, configurable through `BASE_CURRENCY` and defaulting to EUR; it is not a per-user setting, because two members sharing a family would otherwise normalize the same expense to different amounts. The display currency is a profile setting applied to the aggregate, so only one conversion runs per response. Alternatives: `REAL` amounts, rejected for rounding; read-time per-expense conversion, rejected as more queries and unstable history.
